...
var calculateDecimalHashesWithHexConverter = hexConverter => seed => {
  var md5Val = md5(seed);
  return [md5Val.substring(0,16), md5Val.substring(16,32)]
  .map(hexHash => hexConverter(hexHash))
}

var joinNumbers = (int, decimal) => {
...
var combineLatLonWithDecimals = (lat, lon) => decimals => {
...
var calculateGeohash = (lat, lon, dowValue, dateValue) => 
